...
class MenuViewModel: ViewModel() {
    ...
    private val _listMenus = MutableLiveData<Menus>()
    val listMenus: LiveData<Menus> = _listMenus

    private val _isLoading = MutableLiveData<Boolean>()
    val isLoading: LiveData<Boolean> = _isLoading

    fun setMenus(id: String) {
        ...
        client.enqueue(object : Callback<DetailRestaurantResponse> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    _listMenus.value = response.body()?.restaurant?.menus
                } else {
